-- ============================================================
-- VIIT CR ELECTIONS 2026 - STORED PROCEDURES & DATABASE FUNCTIONS (RPC)
-- Database: Supabase PostgreSQL
-- Enforces: Atomic ballot locking, One-Student-One-Vote, Idempotency, Audit
-- ============================================================
...
-- ------------------------------------------------------------
-- PRIMARY ATOMIC RPC FUNCTION: cast_vote
-- ------------------------------------------------------------
...
DECLARE
    ...
    v_receipt JSONB;
BEGIN
    -- 1. Idempotency Check via vote_requests Table
    ...
    -- 2. Identify Authenticated Student
    -- Priority: auth.uid() mapped via students.auth_user_id OR explicit student ID
    ...
    -- 3. Candidate Parameter Validation
    ...
    -- Timing Validation (Server/Database Authority)
    IF v_election.start_time IS NOT NULL AND v_now < v_election.start_time THEN
        RAISE EXCEPTION 'ELECTION_NOT_STARTED: Election start time is %.', v_election.start_time USING ERRCODE = 'P0009';
    END IF;

    IF v_election.end_time IS NOT NULL AND v_now > v_election.end_time THEN
        RAISE EXCEPTION 'ELECTION_CLOSED: Election ended at %.', v_election.end_time USING ERRCODE = 'P0010';
    END IF;
    ...
    -- 6. ONE STUDENT = ONE VOTE Verification (Prevent Duplicate Submissions)
    ...
    -- Section Alignment Check (Strict boundary: Section A student votes only for Section A candidates)
    ...
    -- 8. Generate Tamper-Evident Ballot Reference
    v_vote_ref := generate_vote_reference(v_student.section);

    -- 9. Insert Ballot into Sealed Ledger
    INSERT INTO votes (
        election_id,
        student_id,
        candidate_id,
    ...
    -- 10. Mark Student as Voted in Same Atomic Transaction
    UPDATE students
    SET has_voted = true,
        voted_at = v_now,
        updated_at = v_now
...
-- ------------------------------------------------------------
-- RPC FUNCTION: get_election_results
-- Calculates official aggregate turnout and candidate tallies securely
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- RPC FUNCTION: reset_election_test_data (SUPER_ADMIN ONLY)
-- Resets votes and voter flags for test cycles
-- ------------------------------------------------------------
